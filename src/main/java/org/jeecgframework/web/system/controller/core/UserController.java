...
import org.jeecgframework.web.system.pojo.base.UserMainPage;
...
public class UserController {
	...
	public void delDataPermit(TSUser user) {
		// 同步删除用户角色关联表
		List<UserDataPermitEntity> dataPermitList = systemService.findByProperty(UserDataPermitEntity.class, "userId", user.getId());
		if (dataPermitList.size() >= 1) {
			for (UserDataPermitEntity dataPermit : dataPermitList) {
				systemService.delete(dataPermit);
			}
		}
	}
	/**
	...
	* @throws
	 ...
	 * @return
	 */
	@RequestMapping(params = "datapermit")
	public ModelAndView datapermit(TSUser user, HttpServletRequest req) {
		if (StringUtil.isNotEmpty(user.getId())) {
			user = systemService.getEntity(TSUser.class, user.getId());
			req.setAttribute("user", user);
			//银行
			List<TSDepart> tsList = systemService.getList(TSDepart.class);
			req.setAttribute("depaReplace", RoletoJson.listToReplaceStr(tsList, "departname", "id"));
			//用户
			List<TSUser> userList = systemService.getList(TSUser.class);
			req.setAttribute("userReplace", RoletoJson.listToReplaceStr(userList, "userName", "id"));
		}

		return new ModelAndView("system/user/datapermit");
	}
	/**
	 * 加载权限列表[权限明细]
	 * 
	 * @return
	 */
	@RequestMapping(params = "dataPermitList")
	public ModelAndView dataPermitList(TSUser user, HttpServletRequest req) {
	
		//===================================================================================
		//获取参数
		Object id0 = user.getId();
		//===================================================================================
		//查询-权限明细
	    String hql0 = "from UserDataPermitEntity where 1 = 1 AND userId = ? ";
		try{
			List<UserDataPermitEntity> userDataPermitList1 = systemService.findHql(hql0,id0);
			req.setAttribute("userDataPermitEntityList", userDataPermitList1);
			req.setAttribute("user", user);
		}catch(Exception e){
			logger.info(e.getMessage());
		}
		return new ModelAndView("system/user/dataPermitList");
	}
	 ...
	 * @return
	 */
	@RequestMapping(params = "savedatapermit")
	@ResponseBody
	public AjaxJson savedatapermit(TSUser user,UserMainPage userMainPage, HttpServletRequest request) {
		String id = request.getParameter("id");
		//先删后插
		String del = "delete from t_s_user_datapermit where user_id='"+id+"'";
		systemService.executeSql(del);
		
		int i = 0;
		while(request.getParameter("userDataPermitEntityList["+i+"].datapermitType")!=null){
			String type = request.getParameter("userDataPermitEntityList["+i+"].datapermitType");
			String org  = request.getParameter("userDataPermitEntityList["+i+"].dataOrgId");
			String userId  = request.getParameter("userDataPermitEntityList["+i+"].dataUserId");
			String uuid = UUIDGenerator.generate();
			String sql = "insert into t_s_user_datapermit(id,user_id,datapermit_type,data_org_id,data_user_id)"
					+ "values('"+uuid+"','"+id+"','"+type+"','"+org+"','"+userId+"')";
	...
	@ResponseBody
	public List<TSUser> getOrgList(HttpServletRequest request, HttpServletResponse response) {
		List<Map<String,Object>> departList = systemService.findForJdbc("select id,departname from t_s_depart ");
		RepSendJson.sendJson(departList, response,"UTF-8");
		return null;
	}
	/**
	 * 用户权限
	...
	@ResponseBody
	public List<TSUser> getUserList(HttpServletRequest request, HttpServletResponse response) {
		List<TSUser> userEntityList = systemService.getList(TSUser.class);
		List userList = new ArrayList();
		for(TSUser user : userEntityList){
			Map map = new HashMap();
			map.put("id",user.getId());
			map.put("userName",user.getUserName());
			userList.add(map);
		}
		RepSendJson.sendJson(userList, response);
		return null;
	}
	...
	@ResponseBody
	public ValidForm checkUserName(HttpServletRequest request,HttpServletResponse response) {
		ValidForm v = new ValidForm();
		...
		List<TSUser> custList = systemService.findByProperty(TSUser.class, "userName", userName);
		if(custList.size()>0){
			TSUser user = custList.get(0);
	 ...
	 * @return
	 */
	@RequestMapping(params = "checkRealName")
	@ResponseBody
	public ValidForm checkRealName(HttpServletRequest request,HttpServletResponse response) {
		ValidForm v = new ValidForm();
		...
		String id = oConvertUtils.getString(request.getParameter("id"));
		List<TSUser> custList = systemService.findByProperty(TSUser.class, "realName", realName);
		if(custList.size()>0){
			TSUser user = custList.get(0);
			if( !user.getId().equals(id) ){
				v.setInfo("真实姓名已存在");
...
//	@RequestMapping(params = "getUserList")
//	@ResponseBody
//	public List<TSUser> getUserList(HttpServletRequest request) {
//		//获取产品信息
//		 String hql0 = "FROM TSUser";
//			    List<TSUser> userEntityList = systemService.findHql(hql0,null);
