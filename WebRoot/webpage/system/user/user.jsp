...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" action="userController.do?saveUser">
	...
	<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
     ...
     ${user.userName }
				...
				<input id="userName" class="inputxt" name="userName" validType="t_s_base_user,userName,id" value="${user.userName }" datatype="s2-10" onchange="checkUserName(this)"/>
				<span class="Validform_checktip">用户名范围在2~10位字符</span>
			</c:if></td>
		</tr>
		<tr>
			<td align="right" width="10%" nowrap><label class="Validform_label"> 真实姓名: </label></td>
			<td class="value" width="10%"><input id="realName" class="inputxt" name="realName" value="${user.realName }" datatype="s2-10"  onchange="checkRealName(this)"/> <span class="Validform_checktip">填写个人真实姓名</span></td>
		</tr>
		...
		<tr>
			...
			<td class="value"><input class="inputxt" name="mobilePhone" value="${user.mobilePhone}" datatype="m" errormsg="手机号码不正确!"> <span class="Validform_checktip"></span></td>
		</tr>
		<tr>
			<td align="right"><label class="Validform_label"> 办公电话: </label></td>
			<td class="value"><input class="inputxt" name="officePhone" value="${user.officePhone}" datatype="*" onchange="checkLxdh(this)" errormsg="办公室电话不正确!"> <span class="Validform_checktip"></span></td>
		</tr>
		<tr>
			<td align="right"><label class="Validform_label"> 常用邮箱: </label></td>
			<td class="value"><input class="inputxt" name="email" value="${user.email}" datatype="*" onchange="checkEmail(this)" errormsg="邮箱格式不正确!" ignore="ignore"> <span class="Validform_checktip"></span></td>
		</tr>
	</table>
</t:formvalid>
</body>
 <script type="text/javascript">
	...
	function checkUserName(obj){
		var val = $(obj).val();
		$("#userName").attr("ajaxurl","checkUserName.do?checkRealName&userName="+encodeURI(encodeURI(val))+"&id="+id);
	}
 	...
 	function checkRealName(obj){
		...
		$("#realName").attr("ajaxurl","busstypeController.do?checkRealName&realName="+encodeURI(encodeURI(rlt))+"&id="+id);	
 	}
	...
	function checkEmail(obj){
		...
		if(rlt != null && rlt.length>0){
			...
			$(obj).val("");
			$(obj).next(".Validform_checktip").html("请输入正确的邮箱！");
		}
